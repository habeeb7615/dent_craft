<tr class="damage_row" id="damage_row{{ $damagedArea['id'] }}">
    ...
    <td colspan="2">
        ...
        <script>
            ...
            function showGuards(area_id, el) {
                var damage_row = $("#damage_row"+area_id)
                ...
                var arg = [n,area_id,"'"+option_text+"'",td3];

                var button = '<button class="btn btn-primary icon-btn" type="button" onclick="add_damage_element('+arg+');">Remove</button>';

                var row = '<tr id="damage_row'+area_id+'_'+td3+'"><td>'+td1+'</td><td><input type="hidden" name=damaged_areas['+area_id+']['+number_of_damages+'][guard_id]" value="'+td3+'">'+td2+'</td><td><input type="hidden" name="damaged_areas['+area_id+']['+number_of_damages+'][price]" value="'+td2+'">'+option_text+'</td><td>'+button+'</td></tr>';
            ...
            $("#damage_select{{ $damagedArea['id'] }}").change(function(e){
                showGuards('{{ $damagedArea['id'] }}', this)
            });
            var guards = {!! $damagedArea['guards'] !!}

            guards.forEach(guard => {
                var damage_row = $("#damage_row{{ $damagedArea['id'] }}")
                var price_list = damage_row.find("td > input[list='priceslist']")
                price_list.val(guard.pivot.panel_cost)
                if (guard.id != 1) {
                    $('#damage_select{{ $damagedArea['id'] }}').val(guard.id).trigger('change')
                }
            });
        </script>
        @endpush --}}
        ...
        <script>
            ...
            guards.forEach(guard => {
                // showGuards('{{ $damagedArea->id }}', '#damage_select{{ $damagedArea->id }}')
                var damage_row = $("#damage_row{{ $damagedArea->id }}")
                var price_list = damage_row.find("td > input[list='priceslist']")
                price_list.val(guard.pivot.panel_cost)
                if (guard.id != 1) {
